...
Create Trigger tr_DiemMH On CHITIETDIEM For Insert 
AS
   If( select DIEM
       FROM CHITIETDIEM) >=0 AND ( select DIEM
       FROM CHITIETDIEM) <=10
BEGIN
    print' Nhap dung diem hoc sinh tu 0-10'
	ROLLBACK TRAN
END
...
Create Trigger tr_TuoiHS On HOCSINH For Insert 
AS
   If( select YEAR(GETDATE()-NgaySinh) 
       from HOCSINH) >20 and  (select YEAR(GETDATE()-NgaySinh) 
                               from HOCSINH) <15 
BEGIN
    print' Tuoi hoc sinh phai tu 15-20'
	ROLLBACK TRAN
END



Create Trigger tr_Diem On CHITIETDIEM For Insert 
AS
    If( select DiemTB
	    from HOCPHAN ) <5

BEGIN
    print' Rot mon roi nhe ban! '
	ROLLBACK TRAN
END


Create Trigger tr_SiSo On LOP For Insert 
AS
   If( select COUNT(SiSo)
       from LOP) >41
BEGIN
    print' Si so lop phai <=40'
	ROLLBACK TRAN
END
